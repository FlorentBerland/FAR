...
int write_block(char cmd,char v1,char v2,char v3)
{
    ...
    if (ret != WRITE_BUF_SIZE) {
        if (ret == -1) {
            fprintf(stderr, "Error writing to GoPiGo (errno %i): %s\n", errno, strerror(errno));
        }
        else {
            fprintf(stderr, "Error writing to GoPiGo\n");
        }
        return ret;
    }
    return 1;
...
char read_byte(void)
{
    ...
    if (ret != reg_size) {
        if (ret == -1) {
            fprintf(stderr, "Unable to read from GoPiGo (errno %i): %s\n", errno, strerror(errno));
        }
        else {
            fprintf(stderr, "Unable to read from GoPiGo\n");
        }
        return -1;
    }
